...
#else
		s = evaluate_scheme_expression("(" IGOR_HISTORY_FILE_VAR ")");
#endif
	}
...
#else
/* This returns the output of the scheme expression */
char *evaluate_scheme_expression(char *Sexpr) { // This returns an allocated string which ought to be freed (ultimately)
	char *sexpr = NULL;
...
#else
/* This returns the output of the scheme expression */
char *exit_val_evaluate_scheme_expression(char *sexpr) { // This returns an allocated string which ought to be freed (ultimately)
#if defined(USES_GUILE)
#error Not implemented yet
#elif defined(USES_GAMBIT)
#error Not implemented yet
...
char *igor_get_commandline(int isfile, FILE *f, char *stdprompt, char *contprompt, char *remindprompt, char *history_file) {
	...
	if (!buffer) {
		...
		*buffer = 0;
		...
		prompt = contprompt;
		collecting_s_exp = starts_sexp(buffer);
		sdepth = sexpr_depth(sdepth, buffer);
		depth = strlen(sdepth);
	}
